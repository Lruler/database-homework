...
const User = () => {
  ...
  const save = async (key) => {
    try {
      ...
      if (newData[index].id) {
        Server.modifyUser(newData[index]).then((data) => {
          console.log(data)
        })
      } else {
        Server.addUser(newData[index]).then(() => {
          Server.getUserList().then((res) => {
            let newD = res.data.map((u, i) => {
              return { key: `${i}`, ...u }
            })
  ...
  return (
    <>
      ...
      <Search placeholder="input user id" onSearch={onSearch} style={{ width: 200, marginLeft: 200 }} />
